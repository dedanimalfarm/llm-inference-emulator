...
#### 3.4.1 Поддержка DeepSeek Multi-Head Latent Attention (MLA)

Для современных моделей семейства DeepSeek (V3, V4, R1) вместо стандартного Multi-Query/Grouped-Query Attention (MQA/GQA) применяется сжатие низкого ранга для KV-кэша. Вместо независимого хранения ключей и значений, кэш сжимается в латентный вектор размера `mla_kv_lora_rank` (для DeepSeek-V3 равен 512), а развязанный RoPE-ключ имеет размерность `mla_qk_rope_dim` (равен 64).

Это позволяет эмулятору точно рассчитывать размер KV-кэша без грубых аппроксимаций:
```
kv_per_token_bytes = layers × (mla_kv_lora_rank + mla_qk_rope_dim) × kv_bits_k / 8
```
При использовании 16-битных активаций это даёт ровно те же `layers * 1152` байт на токен, что и GQA-проекция с одной KV-головой и `head_dim=288`, гарантируя преемственность старых расчётов, но обеспечивая физически верное описание оригинальной архитектуры DeepSeek.
...
## Моделирование коммуникационных задержек в распределенном инференсе (TP/PP)

Современные LLM часто не умещаются на одном ускорителе или требуют распределения вычислений для снижения задержки. Эмулятор поддерживает детальное физическое моделирование топологий распределённого инференса с добавлением накладных расходов от сетевого интерконнекта.

### 1. Tensor Parallelism (TP) и Ring All-Reduce

В архитектуре Megatron-LM тензорный параллелизм разделяет веса слоёв (Attention и MLP) горизонтально между $N_{\text{TP}}$ видеокартами.
* **Принцип работы**: На каждом слое модели карты должны дважды синхронизировать активации с помощью операции `All-Reduce` (после проекции внимания и после MLP).
* **Моделирование Ring All-Reduce**: Для кольцевого All-Reduce объём отправляемых и получаемых данных на карту составляет $2 \frac{N_{\text{TP}}-1}{N_{\text{TP}}} \cdot \text{volume}$. С учётом двух синхронизаций за слой, формула оверхеда:
  $$t_{\text{comm\_layer}} = 4(N_{\text{TP}} - 1) \cdot \text{latency} + 4 \cdot \frac{N_{\text{TP}} - 1}{N_{\text{TP}}} \cdot \frac{\text{Tokens} \cdot d_{\text{model}} \cdot 2}{\text{Bandwidth}}$$
  $$t_{\text{TP\_comm}} = \text{layers} \cdot t_{\text{comm\_layer}}$$
* **Prefill vs Decode**: В фазе Prefill обрабатывается весь промпт параллельно ($\text{Tokens} = b_{\text{eff}} \cdot P_{\text{in\_eff}}$). В фазе Decode токенизация идёт последовательно по одному шагу ($\text{Tokens} = b_{\text{eff}}$).
* **Физическое ограничение (Heuristic)**: Данная модель описывает коммуникацию как чисто аддитивную задержку (upper-bound roofline limit), выполняющуюся строго последовательно с вычислениями. В реальных оптимизированных рантаймах (vLLM, Megatron-LM) асинхронное перекрытие вычислений и связи (communication-computation overlap) сокращает фактический штраф в 2-3 раза, что отмечено в предупреждениях UI.

### 2. Pipeline Parallelism (PP) и пузыри планирования (1F1B Bubbles)

Пайплайновый параллелизм разделяет слои модели последовательно между $P_{\text{PP}}$ стадиями.
* **Распределение слоёв**: Время базовых вычислений стадии на prefill и decode уменьшается пропорционально $P_{\text{PP}}$:
  $$t_{\text{stage\_prefill}} = \frac{t_{\text{pre}}}{P_{\text{PP}}}$$
  $$t_{\text{stage\_decode}} = \frac{t_{\text{dec}}}{P_{\text{PP}}}$$
* **Межстадийный перенос**: Передача активаций между границами PP-стадий рассчитывается по объёму тензора размерности $\text{Tokens} \cdot d_{\text{model}} \cdot 2$ байт:
  $$t_{\text{pp\_comm}} = (P_{\text{PP}} - 1) \cdot \left( \text{latency} + \frac{\text{Tokens} \cdot d_{\text{model}} \cdot 2}{\text{Bandwidth}} \right)$$
* **Моделирование пузыря 1F1B (1-Forward-1-Backward)**: Из-за последовательной природы пайплайна на стыках стадий возникают простои (пузыри), пока первая микро-пачка запросов доходит до конца пайплайна, а последующие ещё не начали обрабатываться. Оверхед от пузыря рассчитывается как:
  $$t_{\text{bubble\_prefill}} = (P_{\text{PP}} - 1) \cdot t_{\text{stage\_prefill}}$$
  $$t_{\text{bubble\_decode}} = \frac{P_{\text{PP}} - 1}{b_{\text{eff}}} \cdot t_{\text{stage\_decode}}$$
  Для decode фазы оверхед снижается с увеличением эффективного батча $b_{\text{eff}}$ в полёте, так как конвейер заполняется плотнее.

### 3. Штраф хост-транзита PCIe (Consumer GPU Host-Mediated Penalty)

При использовании тензорного параллелизма (TP) на потребительских десктопных картах (GeForce RTX 3090, 4090, 5090) по шине PCIe возникает критическое ограничение.
* **Причина**: В отличие от серверных чипов (A100, H100) с прямым NVLink-соединением или AMD с Infinity Fabric, видеокарты потребительского класса лишены NVLink-мостов (начиная с RTX 4090). Более того, драйвер NVIDIA GeForce блокирует прямую адресацию P2P DMA (Peer-to-Peer Direct Memory Access) между картами по PCIe.
* **Следствие**: Любой обмен активациями (All-Reduce) вынужден идти транзитом через оперативную память центрального процессора (CPU host transit).
* **Моделирование штрафа**: Для таких конфигураций при `tp_size > 1` и PCIe-интерконнекте эмулятор автоматически применяет пенализацию:
  * Эффективная пропускная способность линка режется на **50%** (половина полосы тратится на чтение в RAM хоста, половина — на запись).
  * Латентность связи утраивается (**3.0x**), так как транзит через CPU добавляет значительные задержки на переключение контекста PCIe.
