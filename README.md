...
1. [`docs/PRIMER.md`](docs/PRIMER.md) — **учебник с нуля** для тех, кто
   хочет понять физику LLM-инференса, прежде чем пользоваться эмулятором.
   5 частей: LLM и токены → архитектура трансформера → prefill/decode/KV-кэш →
   GPU-физика и roofline → обзор движков (vLLM, TensorRT-LLM, llama.cpp,
   SGLang). Академический тон, все формулы выводятся, каждая тема
   привязана к коду репо.
2. [`docs/WALKTHROUGH.md`](docs/WALKTHROUGH.md) — формальный пошаговый
   разбор эмулятора на реальном кейсе (Qwen-2.5-32B Q4_K_M на 2× RTX 3090).
   От пользовательского ввода до сравнения с замерами. С глоссарием.
3. [`docs/VLLM_OPTIMIZATIONS.md`](docs/VLLM_OPTIMIZATIONS.md) — design
   doc по vLLM-стиль оптимизациям с обоснованием каждой корректировки
   формулы.
